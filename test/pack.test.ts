...
test("pnpm pack still ships a gitignored .env when files is *", { skip: hasBin("pnpm") ? false : "pnpm not installed" }, () => {
  const cwd = makeDir({
    "package.json": JSON.stringify({ name: "pnpm-leaky", version: "1.0.0", files: ["*"] }),
    "README.md": "x",
    LICENSE: "MIT",
    ".env": "SECRET=1\n",
  });
...
test("yarn pack still ships a gitignored .env when files is *", { skip: hasBin("yarn") ? false : "yarn not installed" }, () => {
  const cwd = makeDir({
